...
export class DocxDocument {
  ...
  paragraphs({ offset = 0, limit = 500 } = {}) {
    const all = this.#paragraphList()
    const page = all.slice(offset, offset + limit)
    return { total: all.length, offset, returned: page.length, paragraphs: page }
  }

  /**
   * 正文段落的结构化列表（`paragraphs()` 与 `compare()` 共用）。
   * @returns {object[]} 段落信息数组。
   */
  #paragraphList() {
    ...
    return nodes.map((p, index) => paragraphInfo(doc, p, index))
  }

  /**
   * 与另一份文档比较**段落级差异**（只读，不改任何文件，也不写 Word 修订标记）。
   *
   * 比对内容：
   *   - 段落：新增 / 删除 / 修改（再区分「文字变了」「样式变了」还是两者都变）。
   *     用最长公共子序列对齐，因此中间插入一段不会把后面所有段落都报成「修改」；
   *   - 表格：表格个数、行列数变化、逐格文本变化；
   *   - 样式：两份文档用到的**段落样式集合**差异；
   *   - 元数据：标题/作者/时间等字段差异。
   *
   * 明确不做：Word 的修订标记（`w:ins`/`w:del`）与「接受/拒绝修订」——
   * 那要在文档里插入带作者与时间的修订记录，属于另一件事。
   *
   * @param {DocxDocument} other - 另一份已打开的文档。
   * @param {object} [options] - 选项。
   * @param {number} [options.maxItems] - 每类差异最多返回多少条（默认 200）。
   * @param {boolean} [options.includeTables] - 是否比较表格（默认 true）。
   * @param {boolean} [options.includeStyles] - 是否比较样式集合（默认 true）。
   * @param {boolean} [options.includeMetadata] - 是否比较元数据（默认 true）。
   * @param {number} [options.maxCells] - 段落对齐的 DP 规模上限（默认 400 万格，约 16 MB）。
   * @returns {object} 差异报告。
   */
  compare(other, { maxItems = 200, includeTables = true, includeStyles = true, includeMetadata = true, maxCells = 4_000_000 } = {}) {
    if (!other || typeof other.paragraphs !== 'function') {
      throw new OfficeError('INVALID_REQUEST', 'compare 需要另一份已打开的 docx 文档。')
    }
    const before = this.#paragraphList()
    const after = other.#paragraphList()
    const warnings = []
    const { ops, exact } = diffSequence(before.map(paragraphKey), after.map(paragraphKey), maxCells)
    if (!exact) {
      warnings.push({
        code: 'COMPARE_ALIGNMENT_FALLBACK',
        message: `段落数过多（${before.length} / ${after.length}），超出精确对齐的计算上限，已退化为按位置配对：中间插入或删除会让后续段落显示为「修改」。`
      })
    }
    ...
    if (exact) {
      ...
      while (i < ops.length) {
        ...
        // 连续的一段「删 + 增」最可能是「同一段被改过」，交给二次对齐去配对
        const removedBlock = []
        const addedBlock = []
        while (i < ops.length && ops[i].op !== 'equal') {
          if (ops[i].op === 'remove') removedBlock.push(before[ops[i].aIndex])
          else addedBlock.push(after[ops[i].bIndex])
          i += 1
        }
        for (const item of pairChangeBlock(removedBlock, addedBlock)) {
          if (item.kind === 'add') added.push(summarizeParagraph(item.paragraph))
          else if (item.kind === 'remove') removed.push(summarizeParagraph(item.paragraph))
          else changed.push(describeParagraphChange(item.before, item.after))
        }
      }
    } else {
      const common = Math.min(before.length, after.length)
      for (let k = 0; k < common; k += 1) {
        if (paragraphKey(before[k]) !== paragraphKey(after[k])) changed.push(describeParagraphChange(before[k], after[k]))
      }
      for (let k = common; k < before.length; k += 1) removed.push(summarizeParagraph(before[k]))
      for (let k = common; k < after.length; k += 1) added.push(summarizeParagraph(after[k]))
    }

    const tables = includeTables
      ? compareTables(this.tables(), other.tables())
      : { before_count: 0, after_count: 0, added: 0, removed: 0, changed: [], total_changes: 0 }
    const styles = includeStyles ? compareStyles(before, after) : { added: [], removed: [] }
    const metadata = includeMetadata ? compareMetadata(this.metadata(), other.metadata()) : { changed: [] }

    const truncate = (list) => (list.length <= maxItems ? { items: list, truncated: false } : { items: list.slice(0, maxItems), truncated: true })
    const paragraphsAdded = truncate(added)
    const paragraphsRemoved = truncate(removed)
    const paragraphsChanged = truncate(changed)
    const tableChanges = truncate(tables.changed)
    const truncated = paragraphsAdded.truncated || paragraphsRemoved.truncated || paragraphsChanged.truncated || tableChanges.truncated
    if (truncated) {
      warnings.push({ code: 'COMPARE_TRUNCATED', message: `差异条目超过上限 ${maxItems}，只返回前 ${maxItems} 条（报告里的 total_* 是真实总数）。` })
    }

    const totalChanged = changed.length + tables.total_changes + metadata.changed.length
    return {
      identical: added.length === 0 && removed.length === 0 && totalChanged === 0 && styles.added.length === 0 && styles.removed.length === 0,
      paragraphs_before: before.length,
      paragraphs_after: after.length,
      paragraphs: {
        added: paragraphsAdded.items,
        removed: paragraphsRemoved.items,
        changed: paragraphsChanged.items,
        total_added: added.length,
        total_removed: removed.length,
        total_changed: changed.length
      },
      tables: {
        before: tables.before_count,
        after: tables.after_count,
        added: tables.added,
        removed: tables.removed,
        changed: tableChanges.items,
        total_changed: tables.total_changes
      },
      styles,
      metadata,
      max_items: maxItems,
...
/**
 * 段落的比对键：样式 + 文本。样式或文本任一不同就算「不同的段落」。
 * @param {object} p - 段落信息。
 * @returns {string} 比对键。
 */
...
function summarizeParagraph(p) {
  return { index: p.index, style: p.style, text: p.text, outline_level: p.outline_level }
}

/**
 * 把一「删」一「增」描述成一处修改，并说明变的是文字、样式还是两者。
 * @param {object} before - 旧段落。
 * @param {object} after - 新段落。
 * @returns {object} 修改描述。
 */

/**
 * 在一个「删 + 增」区块内做**二次对齐**，决定哪些是「同一段被改过」、哪些是纯粹的新增/删除。
 *
 * 为什么需要这一步：一级对齐用的是「样式 + 文字」整体比对键，一段文字被改过之后
 * 就和原来的自己不再相等，于是一级对齐只能给出一串「删」和「增」。如果直接按顺序配对，
 * 就会把「新插入的段落」说成「旧段落被改了」。
 *
 * 二次对齐按**相似度**打分（文字相似度 ×2 + 样式相同 ×1）做保序最大权匹配：
 *   - 文字几乎一样、只是样式变了 → 判为「改了样式」；
 *   - 样式一样但文字完全不同 → 不会被硬凑成一对（分数过低视作不匹配）。
 *
 * 实测两个方向都对得上 Word 的说法：
 *   A=[标题, 正文(dsh-exp-office)] → B=[标题, 新增段(Heading1), 正文(Harness)]
 *     ⇒ 「新增一段 + 正文文字被改」（而不是「正文被改成新增段 + 多出一段正文」）；
 *   A=[标题, 正文, 正文结束。] → B=[标题, 正文(改), 新增段, 正文结束。(改样式)]
 *     ⇒ 「正文文字被改 + 新增一段 + 正文结束改了样式」。
 *
 * @param {object[]} removedBlock - 一级对齐中的「删」。
 * @param {object[]} addedBlock - 一级对齐中的「增」。
 * @returns {object[]} 逐项结果：`{kind:'change', before, after}` / `{kind:'add', paragraph}` / `{kind:'remove', paragraph}`。
 */
function pairChangeBlock(removedBlock, addedBlock) {
  if (removedBlock.length === 0) return addedBlock.map((p) => ({ kind: 'add', paragraph: p }))
  if (addedBlock.length === 0) return removedBlock.map((p) => ({ kind: 'remove', paragraph: p }))
  ...
  const flush = () => {
    ...
    for (let k = 0; k < pairs; k += 1) out.push({ kind: 'change', before: pendingRemoved[k], after: pendingAdded[k] })
    for (let k = pairs; k < pendingRemoved.length; k += 1) out.push({ kind: 'remove', paragraph: pendingRemoved[k] })
    for (let k = pairs; k < pendingAdded.length; k += 1) out.push({ kind: 'add', paragraph: pendingAdded[k] })
    pendingRemoved = []
    pendingAdded = []
  }

  // 区块过大时不做 O(k²) 打分，退回按位置配对（并让调用方照常给出总数）
  if (removedBlock.length * addedBlock.length > 250_000) {
    return [
      ...removedBlock.map((p, k) => (k < addedBlock.length ? { kind: 'change', before: p, after: addedBlock[k] } : { kind: 'remove', paragraph: p })),
      ...addedBlock.slice(removedBlock.length).map((p) => ({ kind: 'add', paragraph: p }))
    ]
  }

  const n = removedBlock.length
  const m = addedBlock.length
  const width = m + 1
  const dp = new Float64Array((n + 1) * width)
  for (let i = n - 1; i >= 0; i -= 1) {
    for (let j = m - 1; j >= 0; j -= 1) {
      const score = pairScore(removedBlock[i], addedBlock[j])
      const take = score >= 1 ? score + dp[(i + 1) * width + j + 1] : -1
      dp[i * width + j] = Math.max(take, dp[(i + 1) * width + j], dp[i * width + j + 1])
    }
  }
  let i = 0
  let j = 0
  while (i < n && j < m) {
    const score = pairScore(removedBlock[i], addedBlock[j])
    if (score >= 1 && dp[i * width + j] === score + dp[(i + 1) * width + j + 1]) {
      flush()
      out.push({ kind: 'change', before: removedBlock[i], after: addedBlock[j] })
      i += 1
      j += 1
    } else if (dp[(i + 1) * width + j] >= dp[i * width + j + 1]) {
      pendingRemoved.push(removedBlock[i])
      i += 1
    } else {
      pendingAdded.push(addedBlock[j])
      j += 1
    }
  }
  while (i < n) {
...
/**
 * 两个段落配成一对的分数：文字相似度 ×2 + 样式相同 ×1。
 * @param {object} before - 旧段落。
 * @param {object} after - 新段落。
 * @returns {number} 分数（≥ 1 才认为是「同一段被改过」）。
 */
function pairScore(before, after) {
  const styleBonus = (before.style ?? '') === (after.style ?? '') ? 1 : 0
  return textSimilarity(before.text, after.text) * 2 + styleBonus
}

/**
 * 文字相似度：字符二元组的 Dice 系数（0–1）。完全相同为 1，任一为空为 0。
 * @param {string} a - 文本 A。
 * @param {string} b - 文本 B。
 * @returns {number} 相似度。
 */
 ...
 * @returns {object} 修改描述。
 */
function describeParagraphChange(before, after) {
  const styleChanged = before.style !== after.style
  const textChanged = before.text !== after.text
  return {
    index_before: before.index,
    index_after: after.index,
    kind: styleChanged && textChanged ? 'text+style' : styleChanged ? 'style' : 'text',
    style_before: before.style,
    style_after: after.style,
    outline_level_before: before.outline_level,
    outline_level_after: after.outline_level,
 ...
 * 序列差异（最长公共子序列）。
 *
 * DP 表用 `Uint32Array` 铺平存，`(n+1) × (m+1) × 4` 字节；
 * 超过 `maxCells` 时**不硬算**，退化为按位置配对并把 `exact` 标为 false
 * （调用方据此给出警告，而不是让用户以为「后面全被改了」）。
 *
 * @param {string[]} aKeys - 旧序列的比对键。
 * @param {string[]} bKeys - 新序列的比对键。
 * @param {number} maxCells - DP 规模上限。
 * @returns {{ops: object[], exact: boolean}} 操作序列（`equal`/`add`/`remove`）与是否精确。
 */
function diffSequence(aKeys, bKeys, maxCells) {
  const n = aKeys.length
  const m = bKeys.length
  if ((n + 1) * (m + 1) > maxCells) {
    const ops = []
    const common = Math.min(n, m)
    for (let i = 0; i < common; i += 1) ops.push({ op: 'equal', aIndex: i, bIndex: i })
    for (let i = common; i < n; i += 1) ops.push({ op: 'remove', aIndex: i })
    for (let j = common; j < m; j += 1) ops.push({ op: 'add', bIndex: j })
    return { ops, exact: false }
  }
  const width = m + 1
  ...
  for (let i = n - 1; i >= 0; i -= 1) {
    for (let j = m - 1; j >= 0; j -= 1) {
      dp[i * width + j] =
        aKeys[i] === bKeys[j] ? dp[(i + 1) * width + j + 1] + 1 : Math.max(dp[(i + 1) * width + j], dp[i * width + j + 1])
    }
  }
  const ops = []
  let i = 0
  ...
  while (i < n && j < m) {
    if (aKeys[i] === bKeys[j]) {
      ...
      ops.push({ op: 'remove', aIndex: i })
      i += 1
    } else {
      ops.push({ op: 'add', bIndex: j })
      j += 1
    }
  }
  while (i < n) {
...
/**
 * 比较两份文档的表格：个数、行列数、逐格文本。
 * @param {object} a - 旧文档的 `tables()`。
 * @param {object} b - 新文档的 `tables()`。
 * @returns {object} 表格差异。
 */
function compareTables(a, b) {
  ...
  let added = 0
  let removed = 0
  const total = Math.max(a.count, b.count)
  for (let t = 0; t < total; t += 1) {
    const left = a.tables[t]
    const right = b.tables[t]
    if (!left) {
      added += 1
      changed.push({ table: t, kind: 'added', rows: right.rows, columns: right.columns })
      continue
    }
    if (!right) {
      removed += 1
      changed.push({ table: t, kind: 'removed', rows: left.rows, columns: left.columns })
      continue
    }
    if (left.rows !== right.rows || left.columns !== right.columns) {
      changed.push({
        table: t,
        kind: 'shape',
        rows_before: left.rows,
        rows_after: right.rows,
        columns_before: left.columns,
        columns_after: right.columns
      })
    }
    const rows = Math.min(left.rows, right.rows)
    const columns = Math.min(left.columns, right.columns)
    for (let r = 0; r < rows; r += 1) {
      for (let c = 0; c < columns; c += 1) {
        const before = left.cells[r]?.[c]?.text ?? null
        const after = right.cells[r]?.[c]?.text ?? null
        if (before !== after) changed.push({ table: t, row: r + 1, column: c + 1, kind: 'cell', before, after })
      }
    }
  }
  return { before_count: a.count, after_count: b.count, added, removed, changed, total_changes: changed.length }
}

/**
 * 比较两份文档用到的段落样式集合。
 * @param {object[]} before - 旧文档段落。
 * @param {object[]} after - 新文档段落。
 * @returns {{added: string[], removed: string[]}} 样式差异。
 */
function compareStyles(before, after) {
  const setOf = (list) => new Set(list.map((p) => p.style).filter((s) => s !== null && s !== undefined))
...
/**
 * 比较两份文档的元数据字段。
 * @param {object} before - 旧文档元数据。
 * @param {object} after - 新文档元数据。
 * @returns {{changed: object[]}} 字段差异。
 */
