 ...
 *
 * 字体策略：
 *   - 拉丁文本用 **PDF 标准 14 字体**（Helvetica / Times / Courier 四体），无需嵌入，任何阅读器都能打开；
 *   - 含中文等非 WinAnsi 字符时，用 `pdf-font.js` **嵌入一份子集化的 TrueType 字体**
 *     （只带用到的字形，保留原字形编号 + `/CIDToGIDMap`），因此文件仍在几十 KB 量级。
 *   - 两类字体都写 `/ToUnicode`，因此**文本可被提取**（自己与第三方解析器都能读回），
 *     而不是只能「看」不能「读」。
...
import { buildPdfFont, buildType0FontObjects, needsEmbeddedFont } from './pdf-font.js'

export { CJK_FONT_CANDIDATES, findUnencodableChars, resolveCjkFont } from './pdf-font.js'
...
export function buildPdf({
  ...
  let embedded = null
  if (needsFont) embedded = buildPdfFont({ texts: bodyLines, fontPath: cjkFontPath, fontIndex: cjkFontIndex })
  ...
  if (embedded) {
    type0Name = 'F2'
    const built = buildType0FontObjects(embedded, nextNum, { compress, deflate: (buffer) => deflateSync(buffer) })
    objects.push(...built.objects)
    type0Num = built.type0Num
    nextNum = built.nextNum
  }
  pages.forEach((pageLines, index) => {
    ...
    pageLines.forEach((text, lineIndex) => {
      ...
      parts.push(`${useEmbedded ? embedded.encode(text) : literalString(text)} Tj`)
    })
    parts.push('ET')
