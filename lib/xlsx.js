...
/** 一次 `read_sheet` 最多物化的单元格数（超出即报错，不继续扫描）。 */
export const MAX_SHEET_CELLS = 500000

/** 一次 `read_range` 允许的区域单元格数上限（区域是二维数组，必须挡住 A1:XFD1048576 这类请求）。 */
export const MAX_RANGE_CELLS = 200000

/**
 * 一张工作表可安全**编辑**的单元格数上限（默认值）。
 *
 * 读取路径已经改成按区域扫描字节，但写入路径必须把整张工作表解析成 DOM 才能做
 * 「只改目标区间」的最小修改。这条路的实测成本（本机 Node 24 / i5-10300H）：
 *
 * | 工作表规模 | 改 1 格的耗时 | 峰值内存增量 |
 * |---|---|---|
 * | 5 万单元格 | 0.46 s | 约 150 MB |
 * | 10 万单元格 | 0.94 s | 约 265 MB |
 * | 20 万单元格 | 1.70 s | 约 495 MB |
 *
 * 按这个斜率，100 万单元格要 2.4 GB 以上——宿主进程（DSH 本体）会先被 GC 拖死。
 * 因此默认在 30 万单元格处明确拒绝，而不是让用户的 Harness 卡死；
 * 需要更大额度时用插件配置 `maxEditableCells` 调整。
 */
export const MAX_EDITABLE_CELLS = 300000

/** 扫描用的标签针（latin1 比较，与工作表 XML 里的标签名逐字节对应）。 */
const TAG_SHEET_DATA = '<sheetData'
const TAG_ROW = '<row'
const TAG_CELL = '<c'
const TAG_SHEET_PROTECTION = '<sheetProtection'
...
export class Workbook {
   ...
   * 读取工作表的稀疏单元格列表。
   *
   * 走原始字节扫描（见 `scanWorksheet`），**不建整张表的 DOM**：
   * 一张 100 万单元格的表，DOM 需要几 GB，扫描只需要一条 `cells` 数组。
   *
   * @param {string} name - 工作表名。
   * @param {object} [options] - 选项。
   * @param {number} [options.maxCells] - 单元格上限，扫到第 `maxCells + 1` 个立即报错。
   * @returns {object[]} 单元格列表。
   */
  #readCells(name, { maxCells = MAX_SHEET_CELLS } = {}) {
    const sheet = this.sheet(name)
    const buf = this.#pkg.read(sheet.path)
    this.info.sheetProtection.set(name, scanSheetProtection(buf))
    return scanWorksheet(buf, null, this.#scanContext(name, maxCells))
  }

  /**
   * 构造一次扫描的上下文。
   *
   * 样式表与共享字符串表都**只解析一次**：`#sharedStrings()` 每次调用都会重建整张
   * 字符串表，若在逐格回调里直接调用会退化成 O(单元格数 × 字符串数)。
   * 扫描上下文与一次读取同生命周期，因此结果只在这一趟里复用。
   *
   * @param {string} name - 工作表名（错误信息用）。
   * @param {number} maxCells - 本次扫描的单元格上限。
   * @returns {object} 扫描上下文。
   */
  #scanContext(name, maxCells) {
    const memo = new Map()
    ...
    return {
      maxCells,
      sheetName: name,
      date1904: this.info.date1904,
      styles: () => once('styles', () => this.#styles()),
      sst: () => once('sst', () => this.#sharedStrings())
    }
  }
   ...
   * 工作表可安全编辑的单元格数上限。
   *
   * 工具层用插件配置 `maxEditableCells` 覆盖（见 `openWorkbook`），
   * 直接用本类的代码可以自行赋值。
   */
  maxEditableCells = MAX_EDITABLE_CELLS

  /**
   * 写入前的规模闸门。
   *
   * 读取已经改成「按区域扫字节」，但写入必须把整张表解析成 DOM——这是「只改目标区间」
   * 这条最小修改前提的代价。成本与整表规模成正比（实测：5 万格 0.46 s/150 MB、
   * 10 万格 0.94 s/265 MB、20 万格 1.70 s/495 MB），再往上会把宿主进程拖进 GC 泥潭。
   * 因此超过上限时**明确拒绝并说明代价**，而不是让 DSH 本体卡死。
   *
   * @param {string} name - 工作表名。
   * @returns {void}
   */
  #assertEditable(name) {
    const limit = this.maxEditableCells ?? MAX_EDITABLE_CELLS
    if (!(limit > 0)) return
    const sheet = this.sheet(name)
    const count = countCells(this.#pkg.read(sheet.path), limit)
    if (count > limit) {
      throw new OfficeError(
        'MEMORY_LIMIT',
        `工作表「${name}」超过可安全编辑的规模（>${limit} 个单元格，已数到 ${count}）：写入要把整张表解析成 DOM，实测 20 万单元格约 1.7 秒、500 MB 内存。建议先拆表，或只读所需区域后写入新文件；确需在超大表上写入，可用插件配置 maxEditableCells 提高上限（自负内存风险）。`,
        { sheet: name, cells: count, max_editable_cells: limit }
      )
    }
  }
   ...
   * 读取工作表全部单元格。
   *
   * 与 `read_range` 走同一条扫描路径，因此只读操作完全不建 DOM；
   * 超过 `maxCells` 时**在扫描途中**就报错，不会先把整张表物化出来再拒绝。
   *
   * @param {string} name - 工作表名。
   * @param {object} [options] - 选项。
   * @param {number} [options.maxCells] - 单元格上限，默认 {@link MAX_SHEET_CELLS}。
   * @returns {object} 工作表内容。
   */
  readSheet(name, { maxCells = MAX_SHEET_CELLS } = {}) {
    const cells = this.#readCells(name, { maxCells })
   ...
   * 读取指定区域，返回二维数组。
   *
   * 实现要点（「只读取用户要求的区域」这条性能要求的落点）：
   *   - 直接扫描工作表部件的**原始字节**，不建整张表的 DOM；
   *   - 扫到区域最后一行之后的第一个 `<row>` **立即停止**，因此读 100 行的代价
   *     只与这 100 行有关，与整表有多少行无关；
   *   - 共享字符串表与样式表只在真的遇到对应单元格时才解析
   *     （纯数字区域不会去碰 `sharedStrings.xml`）。
   *
   * 区域大小有硬上限 {@link MAX_RANGE_CELLS}：区域会被展开成二维数组，
   * `A1:XFD1048576` 这种请求必须在分配内存之前就拒绝。
   *
   * @param {string} name - 工作表名。
   * @param {string} range - 区域引用，如 `A1:C10`。
   * @param {object} [options] - 选项。
   * @param {number} [options.maxCells] - 区域单元格上限，默认 {@link MAX_RANGE_CELLS}。
   * @returns {object} 区域内容。
   */
  readRange(name, range, { maxCells = MAX_RANGE_CELLS } = {}) {
    ...
    if (area > maxCells) {
      throw new OfficeError(
        'MEMORY_LIMIT',
        `区域 ${range} 需要 ${area} 个单元格（${height} 行 × ${width} 列），超过单次上限 ${maxCells}。请缩小区域，或按行分段读取。`,
        { range, cells: area, max_cells: maxCells }
      )
    }
    const sheet = this.sheet(name)
    const buf = this.#pkg.read(sheet.path)
    const cells = scanWorksheet(buf, bounds, this.#scanContext(name, area))
    const rows = Array.from({ length: height }, () => new Array(width).fill(null))
    for (const cell of cells) {
      rows[cell.row - bounds.start.row][cell.col - bounds.start.col] = {
        ref: cell.ref,
        value: cell.value,
        type: cell.valueType,
  ...
  writeCells(name, edits) {
    ...
    //
    // 行号按 ECMA-376 的规则推算：`<row>` 缺 `r` 时用「上一行 + 1」。
    // 早期版本直接跳过缺 `r` 的行，于是写这种行里的单元格会被当成「新行」，
    // 结果是同一个 `B2` 在文件里出现两次（读回来一个旧值一个新值）——已修。
    const rowNodes = new Map()
    const cellIndex = new Map()
    let impliedRow = 0
    for (const row of findAll(sheetData, 'row')) {
      const rawRow = attr(row, 'r')
      const r = rawRow === undefined ? impliedRow + 1 : Number(rawRow)
      impliedRow = r
      if (!(r > 0)) continue
      if (!rowNodes.has(r)) rowNodes.set(r, row)
      for (const c of row.children ?? []) {
        if (c.type !== 'element' || localName(c) !== 'c') continue
        const ref = attr(c, 'r')
        if (!ref) continue
        const key = `${parseRef(ref).col}:${r}`
        if (!cellIndex.has(key)) cellIndex.set(key, c)
      }
    }
  ...
  findAndReplace({ find: needle, replace: replacement, sheet: sheetName = null, matchCase = false, wholeCell = false, includeFormulas = false, maxReplacements = 100000 }) {
    ...
    for (const sheet of targets) {
      this.#assertEditable(sheet.name)
      const doc = this.#doc(sheet.path)
      const sst = this.#sharedStrings()
...
/**
 * 判断标签名之后的字节是否表示标签名结束（`>`、`/`、空白）。
 *
 * 作用是避免把 `<rowBreaks>` 当成 `<row>`、把 `<cols>` 当成 `<c>`。
 *
 * @param {number} byte - 字节值。
 * @returns {boolean} 是否是边界。
 */
function isTagBoundary(byte) {
  return byte === 0x3e || byte === 0x2f || byte === 0x20 || byte === 0x09 || byte === 0x0a || byte === 0x0d
}

/**
 * 在字节区间 `[from, to)` 内找第一个 `<name ...>` 元素。
 *
 * 只做字节层面的定位，不解码、不建节点。
 *
 * **必须用子视图（`subarray`）把搜索框在区间内**：`Buffer#indexOf` 找不到时会一路
 * 扫到缓冲区结尾，而 `to` 只能事后判断。像 `<is>`（行内字符串）这种在整个工作表里
 * 很少出现的标签，一旦逐格去搜，每格都会把剩下的整张表扫一遍 —— 20 万单元格的表
 * 读 100 行要 15 秒就是这么来的。子视图是零拷贝的，只借用同一段内存。
 *
 * @param {Buffer} buf - 字节。
 * @param {string} name - 标签本地名。
 * @param {number} from - 起始偏移。
 * @param {number} to - 结束偏移。
 * @returns {{start: number, selfClosing: boolean, tagEnd: number, contentStart: number, contentEnd: number}|null} 位置信息。
 */
function findElement(buf, name, from, to) {
  const view = buf.subarray(from, to)
  const needle = `<${name}`
  let pos = 0
  while (pos < view.length) {
    const at = view.indexOf(needle, pos, 'latin1')
    if (at === -1) return null
    if (isTagBoundary(view[at + needle.length])) {
      const tagEnd = view.indexOf(0x3e, at + needle.length)
      if (tagEnd === -1) return null
      const selfClosing = view[tagEnd - 1] === 0x2f
      const contentStart = tagEnd + 1
      return {
        start: from + at,
        selfClosing,
        tagEnd: from + tagEnd,
        contentStart: from + contentStart,
        contentEnd: selfClosing ? from + contentStart : from + findCloseTagIn(view, name, contentStart)
      }
    }
...
/**
 * 找 `</name ...>` 的起始偏移；找不到时返回 `to`。
 * @param {Buffer} buf - 字节。
 * @param {string} name - 标签本地名。
 * @param {number} from - 起始偏移。
 * @param {number} to - 结束偏移。
 * @returns {number} 结束标签起始偏移，或 `to`。
 */
function findCloseTag(buf, name, from, to) {
  return from + findCloseTagIn(buf.subarray(from, to), name, 0)
}

/**
 * 在视图内找 `</name`，找不到时返回视图长度。
 * @param {Buffer} view - 子视图。
 * @param {string} name - 标签本地名。
 * @param {number} from - 视图内起始偏移。
 * @returns {number} 视图内偏移。
 */
function findCloseTagIn(view, name, from) {
  const needle = `</${name}`
  let pos = from
  while (pos < view.length) {
    const at = view.indexOf(needle, pos, 'latin1')
    if (at === -1) return view.length
    if (isTagBoundary(view[at + needle.length])) return at
    pos = at + needle.length
  }
  return view.length
}
...
/**
 * 解析标签上的属性，返回小写名 → 值（已解码实体）。
 *
 * 与 `attr()` 同语义：没有该属性时返回 `undefined`（而不是空串）。
 *
 * @param {Buffer} buf - 字节。
 * @param {number} from - 标签起始（`<`）。
 * @param {number} to - 标签结束（`>` 之后）。
 * @returns {Record<string, string>} 属性表。
 */
function tagAttrs(buf, from, to) {
  const tag = buf.toString('latin1', from, to)
  const attrs = {}
  const pattern = /([A-Za-z_][\w:.-]*)\s*=\s*(?:"([^"]*)"|'([^']*)')/g
  let m = pattern.exec(tag)
  while (m !== null) {
...
/**
 * 取一段 XML 内容的文本，与 `XmlDoc.text()` / `collectText()` 同语义：
 * 去掉标签、解码实体，CDATA 取其内容，注释整段丢弃。
 *
 * 已知且唯一的差异：`<v>` 里出现 CDATA 时，DOM 路径用的正则会把
 * `<![CDATA[...]]>` 一并当标签删掉，这里按 CDATA 语义取内容。
 * `<v>` 只会是数字或共享字符串下标，实际文件里不会出现 CDATA。
 *
 * @param {Buffer} buf - 字节。
 * @param {number} from - 内容起始。
 * @param {number} to - 内容结束。
 * @returns {string} 文本。
 */
function contentText(buf, from, to) {
  if (to <= from) return ''
  const view = buf.subarray(from, to)
  const firstTag = view.indexOf(0x3c)
  if (firstTag === -1) {
    // 绝大多数单元格值（数字、共享字符串下标）都没有标签也没有实体：
    // 直接解码并跳过 decodeEntities，省掉每格两次正则扫描。
    const text = view.toString('utf8')
    return text.includes('&') ? decodeEntities(text) : text
  }
  let out = ''
  let pos = 0
  while (pos < view.length) {
    const lt = view.indexOf(0x3c, pos)
    if (lt === -1) {
      out += view.toString('utf8', pos)
      break
    }
    if (lt > pos) out += view.toString('utf8', pos, lt)
    if (view[lt + 1] === 0x21 && view[lt + 2] === 0x2d && view[lt + 3] === 0x2d) {
      const end = view.indexOf('-->', lt + 4, 'latin1')
      pos = end === -1 ? view.length : end + 3
    } else if (view[lt + 1] === 0x21 && view.toString('latin1', lt, lt + 9) === '<![CDATA[') {
      const end = view.indexOf(']]>', lt + 9, 'latin1')
      out += view.toString('utf8', lt + 9, end === -1 ? view.length : end)
      pos = end === -1 ? view.length : end + 3
    } else {
      const gt = view.indexOf(0x3e, lt)
      if (gt === -1) break
      pos = gt + 1
    }
  }
...
/**
 * 数一张工作表里有多少个单元格，数到 `limit + 1` 就停。
 *
 * 只按字节找 `<c`，并用边界判定把 `<cols>`、`<cfRule>` 这类同前缀标签排除掉；
 * 早停在超大表上让闸门本身的代价可以忽略（24 MB 的表部件约十几毫秒）。
 *
 * @param {Buffer} buf - 工作表部件字节。
 * @param {number} limit - 数到这个数就返回（调用方只关心「是否超过」）。
 * @returns {number} 单元格数（超过 `limit` 时即 `limit + 1`）。
 */
function countCells(buf, limit) {
  let count = 0
  let pos = 0
  while (count <= limit) {
    const at = buf.indexOf(TAG_CELL, pos)
    if (at === -1) return count
    if (isTagBoundary(buf[at + TAG_CELL.length])) count += 1
    pos = at + TAG_CELL.length
  }
  return count
}
 ...
 * 判断工作表是否被保护。
 *
 * `sheetProtection` 按 ECMA-376 的次序排在 `sheetData` **之后**，
 * 因此不能在读完区域就早停的扫描里顺手看，必须单独定位一次。
 *
 * @param {Buffer} buf - 工作表部件字节。
 * @returns {boolean} 是否保护。
 */
function scanSheetProtection(buf) {
  const at = buf.indexOf(TAG_SHEET_PROTECTION)
  if (at === -1) return false
  const tagEnd = buf.indexOf(0x3e, at)
  if (tagEnd === -1) return false
  const attrs = tagAttrs(buf, at, tagEnd + 1)
  return attrs.sheet === '1' || attrs.objects === '1'
}

/**
 * 扫描工作表字节，按需解析单元格。
 *
 * 与「先把整张表建成 DOM、再从中挑出区域」相比，这里在区域最后一行之后的
 * 第一个 `<row>` 就 `break`：读 100 行的代价只与这 100 行有关。
 *
 * 单元格语义与写入路径保持一致（缺省 `t="n"`、缺省 `s="0"`、缺 `r` 的 `<c>` 跳过、
 * 日期按样式表判定），单元格类型分支见 {@link cellFromBytes}。
 *
 * @param {Buffer} buf - 工作表部件字节。
 * @param {{start: {col: number, row: number}, end: {col: number, row: number}}|null} bounds - 区域；`null` 表示整表。
 * @param {object} ctx - 上下文。
 * @param {number} ctx.maxCells - 单元格上限。
 * @param {string} ctx.sheetName - 工作表名（错误信息用）。
 * @param {boolean} ctx.date1904 - 是否 1904 日期系统。
 * @param {() => object} ctx.styles - 惰性取样式表。
 * @param {() => object} ctx.sst - 惰性取共享字符串表。
 * @returns {object[]} 单元格列表（文档顺序）。
 */
function scanWorksheet(buf, bounds, ctx) {
  const cells = []
  const dataStart = buf.indexOf(TAG_SHEET_DATA)
  if (dataStart === -1) return cells
  const dataTagEnd = buf.indexOf(0x3e, dataStart + TAG_SHEET_DATA.length)
  if (dataTagEnd === -1) {
    throw new OfficeError('CORRUPTED_DOCUMENT', `工作表「${ctx.sheetName}」的 sheetData 起始标签不完整。`)
  }
  if (buf[dataTagEnd - 1] === 0x2f) return cells
  const dataEnd = findCloseTag(buf, 'sheetData', dataTagEnd + 1, buf.length)
  let impliedRow = 0
  let pos = dataTagEnd + 1
  while (pos < dataEnd) {
    const rowStart = buf.indexOf(TAG_ROW, pos)
    if (rowStart === -1 || rowStart >= dataEnd) break
    if (!isTagBoundary(buf[rowStart + TAG_ROW.length])) {
      pos = rowStart + TAG_ROW.length
      continue
    }
    const rowTagEnd = buf.indexOf(0x3e, rowStart + TAG_ROW.length)
    if (rowTagEnd === -1 || rowTagEnd >= dataEnd) {
      throw new OfficeError('CORRUPTED_DOCUMENT', `工作表「${ctx.sheetName}」存在不完整的 <row> 起始标签。`)
    }
    const attrs = tagAttrs(buf, rowStart, rowTagEnd + 1)
    const rowNumber = attrs.r === undefined ? impliedRow + 1 : Number(attrs.r)
    impliedRow = rowNumber
    const selfClosing = buf[rowTagEnd - 1] === 0x2f
    const contentStart = rowTagEnd + 1
    const rowEnd = selfClosing ? contentStart : findCloseTag(buf, 'row', contentStart, dataEnd)
    if (bounds && rowNumber > bounds.end.row) break
    if (!bounds || rowNumber >= bounds.start.row) {
      scanRowCells(buf, contentStart, rowEnd, cells, ctx, bounds)
    }
    pos = rowEnd > contentStart ? rowEnd : contentStart
  }
  return cells
}

/**
 * 扫描一行里的全部 `<c>`，把落在区域内的单元格追加到 `cells`。
 * @param {Buffer} buf - 工作表部件字节。
 * @param {number} from - 行内容起始。
 * @param {number} to - 行内容结束。
 * @param {object[]} cells - 结果数组。
 * @param {object} ctx - 扫描上下文。
 * @param {{start: {col: number, row: number}, end: {col: number, row: number}}|null} bounds - 区域；`null` 表示整表。
 * @returns {void}
 */
function scanRowCells(buf, from, to, cells, ctx, bounds) {
  let pos = from
  while (pos < to) {
    const cellStart = buf.indexOf(TAG_CELL, pos)
    if (cellStart === -1 || cellStart >= to) return
    if (!isTagBoundary(buf[cellStart + TAG_CELL.length])) {
      pos = cellStart + TAG_CELL.length
      continue
    }
    const tagEnd = buf.indexOf(0x3e, cellStart + TAG_CELL.length)
    if (tagEnd === -1 || tagEnd >= to) return
    const selfClosing = buf[tagEnd - 1] === 0x2f
    const contentStart = tagEnd + 1
    const cellEnd = selfClosing ? contentStart : findCloseTag(buf, 'c', contentStart, to)
    const cell = cellFromBytes(buf, cellStart, tagEnd, contentStart, cellEnd, ctx)
    if (cell && insideBounds(cell, bounds)) {
      cells.push(cell)
      if (cells.length > ctx.maxCells) {
        throw new OfficeError(
          'MEMORY_LIMIT',
          `工作表「${ctx.sheetName}」的单元格数超过单次上限 ${ctx.maxCells}（已扫到 ${cell.ref}）。请改用 office_read_range 按区域读取。`,
          { sheet: ctx.sheetName, max_cells: ctx.maxCells, ref: cell.ref }
        )
      }
    }
    pos = cellEnd > contentStart ? cellEnd : contentStart
  }
}
 ...
 * 判断单元格是否落在区域内。
 *
 * 以单元格自己的 `r` 为准（而不是所在 `<row>` 的行号）：缺 `r` 的 `<row>` 行号是
 * 推算出来的，显式引用更可靠。
 *
 * @param {object} cell - 单元格。
 * @param {{start: {col: number, row: number}, end: {col: number, row: number}}|null} bounds - 区域。
 * @returns {boolean} 是否在区域内。
 */
function insideBounds(cell, bounds) {
  if (!bounds) return true
  return cell.row >= bounds.start.row && cell.row <= bounds.end.row && cell.col >= bounds.start.col && cell.col <= bounds.end.col
}

/**
 * 把一个 `<c>` 字节区间解析为值对象，`null` 表示该单元格应被跳过。
 *
 * 类型判定分支与写入路径的语义一一对应：`t="s"` 查共享字符串表，
 * `inlineStr` 取 `<is>` 文本，`str`/`b`/`e` 取 `<v>`，其余按数字处理并按样式判日期。
 *
 * @param {Buffer} buf - 工作表部件字节。
 * @param {number} cellStart - `<c` 起始偏移。
 * @param {number} tagEnd - 起始标签的 `>` 偏移。
 * @param {number} contentStart - 内容起始偏移。
 * @param {number} contentEnd - 内容结束偏移。
 * @param {object} ctx - 扫描上下文。
 * @returns {object|null} 单元格值对象。
 */
function cellFromBytes(buf, cellStart, tagEnd, contentStart, contentEnd, ctx) {
  const attrs = tagAttrs(buf, cellStart, tagEnd + 1)
  const ref = attrs.r
  if (!ref) return null
  ...
  const fNode = findElement(buf, 'f', contentStart, contentEnd)
  const vNode = findElement(buf, 'v', contentStart, contentEnd)
  const isNode = findElement(buf, 'is', contentStart, contentEnd)
  const formula = fNode ? contentText(buf, fNode.contentStart, fNode.contentEnd) : null
  const valueText = vNode ? contentText(buf, vNode.contentStart, vNode.contentEnd) : null

  let value = null
  let valueType = 'number'
  if (type === 's') {
    const idx = Number(valueText === null ? -1 : valueText)
    value = ctx.sst().items[idx] ?? null
    valueType = 'string'
  } else if (type === 'inlineStr') {
    value = isNode ? contentText(buf, isNode.contentStart, isNode.contentEnd) : null
    valueType = 'string'
  } else if (type === 'str') {
    value = valueText
    valueType = 'string'
  } else if (type === 'b') {
    value = valueText === null ? false : valueText === '1'
    valueType = 'boolean'
  } else if (type === 'e') {
    value = valueText
    valueType = 'error'
    ...
    if (isDateFormat(styleIndex, ctx.styles())) {
      value = serialToIso(num, ctx.date1904)
      valueType = 'date'
    } else {
      value = num
      valueType = 'number'
