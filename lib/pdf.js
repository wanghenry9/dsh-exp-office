...
import { buildPdfFont, buildType0FontObjects, needsEmbeddedFont as labelNeedsEmbeddedFont } from './pdf-font.js'
...
export class PdfDocument {
   ...
   *
   * 字体：纯 WinAnsi 文本用 base-14 的 Helvetica（阅读器内置，不需要嵌入）；
   * **含中文等非 WinAnsi 字符时自动嵌入一份子集化的 TrueType 字体**（`pdf-font.js`），
   * 因此中文水印/页码也能画出来并且可被提取 —— 只是文件会大一些（字体子集只带用到的字形）。
   *
   ...
   * @param {string} [args.cjkFontPath] - 嵌入用的中文字体；省略时自动找本机字体。
   * @returns {object} 变更信息。
   */
  addTextOverlay({ text, pages = null, position = 'center', fontSize = null, gray = 0.9, margin = 24, labelFor = null, cjkFontPath = null }) {
    ...
    // 先把所有页的文本算出来：嵌入字体时要一次收集全部字符，只做一份子集
    const labels = targets.map((index, order) => labelAt(index, order))
    const needsFont = labels.some((label) => labelNeedsEmbeddedFont(label))

    let fontRef
    let fontName = 'Wm'
    let embeddedFont = null
    if (needsFont) {
      embeddedFont = buildPdfFont({ texts: labels, fontPath: cjkFontPath })
      // 六个互相引用的对象必须先预留真实对象号，否则内部引用会指向别的对象
      // （踩过：ToUnicode 指向了一个页面内容流，文本提取因此全是 \u0000）
      const reserved = this.#reserveNums(6)
      const startNum = Number(reserved[0].split(' ')[0])
      const built = buildType0FontObjects(embeddedFont, startNum, {
        compress: true,
        deflate: (buffer) => zlib.deflateSync(buffer)
      })
      built.objects.forEach((object, index) => this.#setObjectAt(reserved[index], object.value))
      fontRef = reserved[0]
      fontName = 'WmCJK'
    } else {
      // 只需要一个 Helvetica 字体对象：所有页共用
      fontRef = this.#addObject({ Type: 'Font', Subtype: 'Type1', BaseFont: 'Helvetica', Encoding: 'WinAnsiEncoding' })
    }
    ...
    for (const [order, index] of targets.entries()) {
      ...
      const useEmbedded = embeddedFont !== null && labelNeedsEmbeddedFont(label)
      const commands = buildOverlayCommands({
        label,
        position,
        size,
        gray,
        ...
        height,
        fontName,
        encoded: useEmbedded ? embeddedFont.encode(label) : null
      })

      const streamRef = this.#addStream(commands)
      const resourcesRef = this.#withFontResource(page, fontRef, fontName)
    ...
    return {
      ...
      embedded_font: embeddedFont
        ? { name: embeddedFont.fontName, characters: embeddedFont.charCount, glyphs: embeddedFont.glyphCount, subset_bytes: embeddedFont.subset.length }
        : null,
   ...
   * 预留连续的 `count` 个对象号。
   *
   * 用于**一次构造多个互相引用的对象**（例如嵌入字体的 6 个对象：Type0 指向 CIDFont、
   * CIDFont 指向 FontDescriptor/CIDToGIDMap、Descriptor 指向 FontFile2）：
   * 只有先知道真实对象号，才能把内部引用写对。预留后必须用 {@link PdfDocument#setObjectAt} 填满，
   * 否则会产生空洞（对象表里缺号是合法的，但没必要留）。
   *
   * @param {number} count - 需要几个号。
   * @returns {string[]} `"num 0"` 列表。
   */
  #reserveNums(count) {
    const first = Number(this.#nextNum().split(' ')[0])
    return Array.from({ length: count }, (_, index) => `${first + index} 0`)
  }

  /**
   * 把对象登记到**指定**对象号（配合 {@link PdfDocument#reserveNums}）。
   * @param {string} key - `"num gen"`。
   * @param {unknown} value - 对象值。
   * @returns {void}
   */
  #setObjectAt(key, value) {
    this.#objects.set(key, { num: Number(key.split(' ')[0]), gen: 0, value })
    this.#stage(key, value)
  }
   ...
   */
  #withFontResource(page, fontRef, name = 'Wm') {
    ...
    // 合并原有字体条目（可能是直接字典，也可能是引用），并把叠加用的字体内联在同一层
    const existingFonts = this.deref(effective?.Font)
    const fonts = {}
    if (existingFonts && typeof existingFonts === 'object') {
      for (const [fontName, ref] of Object.entries(existingFonts)) {
        if (fontName.startsWith('__')) continue
        fonts[fontName] = ref
      }
    }
    if (fonts[name] === undefined) fonts[name] = { ref: fontRef }
    copy.Font = fonts
 ...
 */
function buildOverlayCommands({ label, position, size, gray, margin, width, height, fontName = 'Wm', encoded = null }) {
  // `encoded` 是嵌入字体时的 Identity-H 十六进制串（每字符 2 字节 CID）；纯 WinAnsi 文本仍用字面串。
  const literal = encoded ?? serializePdfBuffer(Buffer.from(label, 'latin1'))
  const estimated = label.length * size * (encoded ? 1 : 0.5)
