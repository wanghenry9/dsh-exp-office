...
export function createOfficeTools({ defineTool, config = {} }) {
  ...
  tools.push(
    defineTool({
      ...
      description:
        '**从零生成**一份 PDF（不依赖任何模板或第三方库）：自写页面树、内容流、字体资源与 xref，同时写入 /ToUnicode，因此生成的文本**可被提取**（自己与第三方解析器都能读回）。使用 PDF 标准 14 字体（Helvetica/Times/Courier），无需嵌入字体；代价是**只支持 WinAnsi 字符集**（拉丁字母/数字/常见标点），中文等会被明确拒绝而不是画成乱码。支持自动折行与分页。',
      parameters: {
        ...
        font_family: { type: 'string', enum: ['helvetica', 'times', 'courier'], description: '字族，默认 helvetica。' },
            ...
            return {
              ...ok({
                ...
                data: {
                  ...
                  orientation: args.orientation ?? 'portrait',
                  encoding: 'WinAnsiEncoding（标准 14 字体，无需嵌入）',
                  text_extractable: true
                }
              }),
              summary: `📄 已生成 PDF：${basename(target)}｜${built.pages} 页 / ${built.lines} 行｜${committed.size} 字节`
            }
          } finally {
            await temp.dispose()
          }
