...
export function createOfficeTools({ defineTool, config = {} }) {
  ...
  tools.push(
    defineTool({
      ...
      description:
        '给 PDF 加水印文字（支持中文）。采用**叠加式编辑**：原内容流一个字节都不改，只新建一个绘制流追加到页面的 /Contents 之后，并用浅灰填充近似淡化（不使用透明度组）。纯拉丁文本用阅读器内置的 Helvetica（无需嵌入字体）；**含中文等非 WinAnsi 字符时自动嵌入一份子集化的 TrueType 字体**，因此中文水印既能画出来也能被提取。默认斜向 45° 居中，可用 {page}/{total} 之类占位符做页码式水印。',
      parameters: {
        path: { type: 'string', required: true, description: 'PDF 文件路径。' },
        text: { type: 'string', required: true, description: '水印文字（支持中文）；支持 {page} 与 {total} 占位符。' },
        cjk_font_path: { type: 'string', description: '嵌入用的中文字体（.ttf/.ttc）；省略时自动找本机字体。' },
          ...
          return await mutatePdf({
            ...
            mutate: async ({ pdf, changes }) => {
              ...
              return {
                ...
                text: args.text,
                embedded_font: result.embedded_font
              }
            },
            summaryOf: (_s, _c, extra) =>
              `💧 已给 ${extra.pages.length} 页添加水印「${extra.text}」（叠加式，原内容流未改动${extra.embedded_font ? `，嵌入字体子集 ${extra.embedded_font.name}` : ''}）。`
          })
        } catch (err) {
          return failure(requestId, err)
        }
  ...
  tools.push(
    defineTool({
      ...
      description:
        '给 PDF 加页码（支持中文，如「第 {page} 页 / 共 {total} 页」）。与水印同一套叠加式写入：新建绘制流追加到页面 /Contents 之后，原内容流不改动；纯拉丁用内置 Helvetica，含中文时自动嵌入字体子集。页码默认放在页脚居中，`start_at` 可指定起始编号（例如从第 5 页开始标 1），页脚已有内容时可用 `margin` 抬高或改用 top。',
      parameters: {
        path: { type: 'string', required: true, description: 'PDF 文件路径。' },
        ...
        try {
          ...
          return await mutatePdf({
            ...
            mutate: async ({ pdf, changes }) => {
              ...
              changes.push(result)
              return { pages: result.pages, start_at: startAt, format, embedded_font: result.embedded_font }
            },
            summaryOf: (_s, _c, extra) =>
              `🔢 已给 ${extra.pages.length} 页添加页码（格式「${extra.format}」，起始 ${extra.start_at}${extra.embedded_font ? `，嵌入字体子集 ${extra.embedded_font.name}` : ''}）。`
          })
        } catch (err) {
          return failure(requestId, err)
        }
